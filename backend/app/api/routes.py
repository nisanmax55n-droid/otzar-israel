...
def works(category: str | None = None, q: str | None = None, db: Session = Depends(get_db)):
    stmt = select(Work).order_by(Work.category, Work.title_he)
    if category:
...
def segments(work_id: int, offset: int = 0, limit: int = Query(200, le=1000), db: Session = Depends(get_db)):
    ...
    return list(db.scalars(select(TextSegment).where(TextSegment.version_id == version.id).order_by(TextSegment.position).offset(offset).limit(limit)).all())

@router.get("/search", response_model=list[SearchHit])
def search(q: str = Query(min_length=2), limit: int = Query(50, le=200), db: Session = Depends(get_db)):
    nq = normalize_hebrew(q)
    stmt = (select(TextSegment, Work).join(TextVersion, TextSegment.version_id == TextVersion.id).join(Work, TextVersion.work_id == Work.id).where(TextVersion.license_verified.is_(True), TextSegment.normalized_text.contains(nq)).limit(limit))

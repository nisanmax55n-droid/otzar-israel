...
def stats(db: Session = Depends(get_db)):
    ...
    if torah_books:
        categories.insert(0, {"name": "Tanakh", "count": torah_books})
    return {
        "works": works_count + torah_books,
        "versions": versions_count,
        "segments": segments_count + torah_verses,
...
def categories(db: Session = Depends(get_db)):
    ...
    torah_books = db.scalar(select(func.count(TorahBook.id))) or 0
    if torah_books:
        result.insert(0, {"name": "Tanakh", "count": torah_books})
    return result
...
def torah_chapter(slug: str, chapter: int, db: Session = Depends(get_db)):
    ...
    verses = list(db.scalars(
        select(TorahVerse)
        .where(TorahVerse.book_id == book.id, TorahVerse.chapter == chapter)
        .order_by(TorahVerse.verse)
    ).all())
...
def torah_book_view(slug: str, db: Session = Depends(get_db)):
    ...
    verses = list(db.scalars(
        select(TorahVerse)
        .where(TorahVerse.book_id == book.id)
        .order_by(TorahVerse.chapter, TorahVerse.verse)
    ).all())
    return [{"id": v.id, "chapter": v.chapter, "verse": v.verse, "text": v.text_nikkud, "ref": v.sefaria_ref} for v in verses]
...
def search(q: str = Query(min_length=2), limit: int = Query(50, le=200), db: Session = Depends(get_db)):
    ...
    hits = [SearchHit(work_id=w.id, work_title=w.title_he, ref=s.ref, text=s.text) for s, w in db.execute(generic_stmt).all()]
    return hits
